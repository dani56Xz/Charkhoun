...
async def callback_handler(update: Update, context: ContextTypes):
    ...
    try:
        if query.data == "back":
            ...
            # ایجاد لیست برترین‌ها با تزئینات زیبا
            msg = "🏆 𝗧𝗢𝗣 𝗪𝗜𝗡𝗡𝗘𝗥𝗦 𝗟𝗘𝗔𝗗𝗘𝗥𝗕𝗢𝗔𝗥𝗗 🏆\n\n"
            msg += "┌────────────┬─────────────────────┬─────────────────┐\n"
            ...
            for i, row in enumerate(rows, 1):
                ...
                # تولید نام نمایشی بدون @ و فقط آیدی عددی
                username_display = f"آیدی: {user_id_val}"
                
                # اضافه کردن مدال‌های مختلف بر اساس رتبه
                if i == 1:
                    medal = "🥇 1st"
                    username_display = f"🏆 𝐂𝐇𝐀𝐌𝐏𝐈𝐎𝐍 🏆"
                elif i == 2:
                    medal = "🥈 2nd"
                    username_display = f"⚡ 𝐕𝐈𝐂𝐄 𝐂𝐇𝐀𝐌𝐏𝐈𝐎𝐍 ⚡"
                elif i == 3:
                    medal = "🥉 3rd"
                    username_display = f"🌟 𝐓𝐎𝐏 𝐏𝐋𝐀𝐘𝐄𝐑 🌟"
                else:
                    medal = f"🏅 {i}th"
                
                # قالب‌بندی زیبا برای هر ردیف
                rank_col = f"│ {medal:<10} │"
                user_col = f" {username_display:<19} │"
                earnings_col = f" {total_earnings:,}{' '*(15-len(str(total_earnings)))} تومان │"
                
                msg += f"{rank_col}{user_col}{earnings_col}\n"
            ...
            if not rows:
                ...
                msg += "├─────────────────────────────────────────────┤\n"
                msg += "│  🎯 اولین برنده این لیست باش و تاریخ ساز شو!  │\n"
                msg += "└─────────────────────────────────────────────┘\n"
            else:
                msg += "└────────────┴─────────────────────┴─────────────────┘\n\n"
                msg += "💫 تو هم می‌تونی یکی از برترین‌ها باشی!\n"
