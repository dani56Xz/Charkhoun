...
async def callback_handler(update: Update, context: ContextTypes):
    ...
    try:
        if query.data == "back":
            ...
            # ایجاد لیست برترین‌ها با فرمت جدید
            msg = "🏆 لیست پر درآمد ها 🏆\n\n"
            
            if not rows:
                msg += "📊 هیچ داده‌ای موجود نیست\n"
                msg += "🎯 اولین برنده این لیست باش و تاریخ ساز شو!"
            else:
                # تعریف مدال‌ها برای رتبه‌های مختلف
                medals = {
                    1: "🥇",
                    2: "🥈", 
                    3: "🥉"
                ...
                for i, row in enumerate(rows, 1):
                    ...
                    # انتخاب مدال یا الماس بر اساس رتبه
                    if i in medals:
                        medal = medals[i]
                    else:
                        medal = "💎"
                    ...
                    # قالب‌بندی جدید
                    if i <= 3:
                        msg += f"{medal} نفر {i}ام: {user_id_val} — 💰 {total_earnings:,} تومان\n"
                    else:
                        msg += f"{medal} نفر {i}ام: {user_id_val} — 💰 {total_earnings:,} تومان\n"
